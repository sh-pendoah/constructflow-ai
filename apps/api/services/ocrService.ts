...
class AzureFormRecognizerProvider implements OCRProvider {
  ...
  async processDocument(filePath: string, mimeType: string): Promise<OCRResult> {
    ...
    try {
          ...
          if (page.lines) {
            for (const line of page.lines) {
              ...
              if (line.polygon && line.polygon.length >= 4) {
                ...
                const xCoords = points.filter((_: any, i: number) => i % 2 === 0).map((p: any) => p.x || p);
                const yCoords = points.filter((_: any, i: number) => i % 2 === 1).map((p: any) => p.y || p);
      ...
      logger.info(`Azure Form Recognizer completed with confidence: ${averageConfidence}`);
      ...
      throw new Error(`Azure Form Recognizer processing failed: ${errorMessage}`);
    }
  }
}
...
export interface LLMProvider {
  name: string;
  extractStructuredData(ocrText: string, documentType: string, schema: any): Promise<Record<string, any>>;
}
...
class AzureOpenAIProvider implements LLMProvider {
  ...
  async extractStructuredData(ocrText: string, documentType: string, schema: any): Promise<Record<string, any>> {
  ...
  private buildExtractionPrompt(ocrText: string, documentType: string, schema: any): string {
...
Extracted Data:`;
  ...
  private async callAzureOpenAI(prompt: string): Promise<string> {
    const url = `${this.endpoint}/openai/deployments/${this.deployment}/chat/completions?api-version=2023-12-01-preview`;
...
class MockLLMProvider implements LLMProvider {
  ...
  async extractStructuredData(ocrText: string, documentType: string, schema: any): Promise<Record<string, any>> {
    ...
    switch (documentType) {
      ...
      case 'compliance':
        return {
          vendor: { value: 'XYZ Insurance', confidence: 0.90 },
...
class AzureDocumentIntelligenceProvider implements OCRProvider {
  ...
  async processDocument(filePath: string, mimeType: string): Promise<OCRResult> {
    ...
    try {
      ...
      const url = `${this.endpoint}/formrecognizer/documentModels/prebuilt-read:analyze?api-version=2023-07-31`;
        ...
        if (page.lines) {
          for (const line of page.lines) {
            ...
            if (line.polygon && line.polygon.length >= 4) {
              ...
              const xCoords = line.polygon.filter((_: any, i: number) => i % 2 === 0);
              const yCoords = line.polygon.filter((_: any, i: number) => i % 2 === 1);
  ...
  private makeHttpsRequest(url: string, options: any, formData: any): Promise<string> {
...
class OCRService {
  ...
  private initializeOCRProvider(): OCRProvider {
    ...
    switch (providerName.toLowerCase()) {
      case 'azure-form-recognizer':
        if (!process.env.AZURE_FORM_RECOGNIZER_ENDPOINT || !process.env.AZURE_FORM_RECOGNIZER_API_KEY) {
          logger.warn('Azure Form Recognizer credentials not set, falling back to Document Intelligence');
          ...
          logger.warn('Azure Document Intelligence credentials not set either, falling back to mock');
      ...
      case 'azure-document-intelligence':
        if (!process.env.AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT || !process.env.AZURE_DOCUMENT_INTELLIGENCE_API_KEY) {
          logger.warn('Azure Document Intelligence credentials not set, falling back to mock');
          return new MockOCRProvider();
        }
        return new AzureDocumentIntelligenceProvider(
          process.env.AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT,
  ...
  async processDocument(filePath: string, mimeType: string): Promise<OCRResult> {
    try {
      logger.info(`Processing document with ${this.ocrProvider.name}: ${filePath}`);
  ...
  async extractInvoiceData(ocrResult: OCRResult): Promise<InvoiceData> {
    try {
      ...
      const extractedData = await this.llmProvider.extractStructuredData(ocrResult.text, 'invoice', schema);

      // Add bounding boxes if available
      if (extractedData.invoiceNumber && ocrResult.blocks) {
        extractedData.invoiceNumber.location = this.findBoundingBox(ocrResult.blocks, extractedData.invoiceNumber.value);
      }
      if (extractedData.vendor && ocrResult.blocks) {
        extractedData.vendor.location = this.findBoundingBox(ocrResult.blocks, extractedData.vendor.value);
      }
  ...
  async extractDailyLogData(ocrResult: OCRResult): Promise<DailyLogData> {
    try {
      ...
      const extractedData = await this.llmProvider.extractStructuredData(ocrResult.text, 'daily-log', schema);
  ...
  async extractComplianceData(ocrResult: OCRResult): Promise<ComplianceData> {
    try {
      ...
      const extractedData = await this.llmProvider.extractStructuredData(ocrResult.text, 'compliance', schema);
   ...
   */
  private findBoundingBox(blocks: Array<{ text: string; boundingBox?: any }>, searchText: string): { x: number; y: number; width: number; height: number } | undefined {
    // Simple text matching - could be improved with fuzzy matching
    const normalizedSearch = searchText.toLowerCase().trim();

    for (const block of blocks) {
      if (block.text.toLowerCase().includes(normalizedSearch) && block.boundingBox) {
  ...
  getConfidenceThreshold(documentType: string): number {
    ...
    return thresholds[documentType] || 0.70;
  }
