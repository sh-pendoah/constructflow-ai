 ...
 */
router.get('/', authMiddleware, tenantMiddleware, async (req: any, res: Response) => {
  const tenantReq = req as TenantRequest;
  try {
    const { documentType, status, priority, page = '1', limit = '50' } = tenantReq.query;
    ...
    const pageNum = Number.isNaN(rawPage) || rawPage < 1 ? DEFAULT_PAGE : rawPage;
    let limitNum = Number.isNaN(rawLimit) || rawLimit < 1 ? DEFAULT_LIMIT : rawLimit;
 ...
 */
router.get('/stats', authMiddleware, tenantMiddleware, async (req: any, res: Response) => {
  const tenantReq = req as TenantRequest;
  try {
    const tenantId = tenantReq.tenantId;
 ...
 */
router.get('/:id', authMiddleware, tenantMiddleware, async (req: any, res: Response) => {
  const tenantReq = req as TenantRequest;
  try {
 ...
 */
router.patch('/:id/assign', authMiddleware, tenantMiddleware, async (req: any, res: Response) => {
  const tenantReq = req as TenantRequest;
  try {
    const { id } = tenantReq.params;
    ...
    logger.info(`Review queue item ${id} assigned to ${assignedTo || tenantReq.user!.id}`);
    res.json(item);
  } catch (error) {
 ...
 */
router.post('/:id/corrections', authMiddleware, tenantMiddleware, async (req: any, res: Response) => {
  const tenantReq = req as TenantRequest;
  try {
    const { id } = tenantReq.params;
    ...
    if (!field || correctedValue === undefined) {
      return res.status(400).json({ message: 'Field and correctedValue are required' });
    }
 ...
 */
router.post('/:id/approve', authMiddleware, tenantMiddleware, async (req: any, res: Response) => {
  const tenantReq = req as TenantRequest;
  try {
    const { id } = tenantReq.params;
    ...
    const item = await ReviewQueueItem.findOne({ _id: id, company: tenantId });
      ...
      switch (item.documentType) {
        ...
        case 'compliance': {
          ...
          if (doc) {
            ...
            await doc.save();
            logger.info(`Compliance doc ${item.documentId} approved and updated`);
          }
          break;
        }
      }
    } catch (docError) {
      logger.error(`Failed to update source document for ${item.documentType}:`, docError);
 ...
 */
router.post('/:id/reject', authMiddleware, tenantMiddleware, async (req: any, res: Response) => {
  const tenantReq = req as TenantRequest;
  try {
    const { id } = tenantReq.params;
    const { notes } = tenantReq.body;
      ...
      switch (item.documentType) {
        ...
        case 'compliance': {
          ...
          if (doc) {
            ...
            await doc.save();
            logger.info(`Compliance doc ${item.documentId} rejected and updated`);
          }
          break;
        }
      }
 ...
 */
router.post('/:id/request-correction', authMiddleware, tenantMiddleware, async (req: any, res: Response) => {
  const tenantReq = req as TenantRequest;
  try {
    const { id } = tenantReq.params;
    const { notes } = tenantReq.body;
 ...
 */
router.delete('/:id', authMiddleware, tenantMiddleware, async (req: any, res: Response) => {
  const tenantReq = req as TenantRequest;
  try {
    const { id } = tenantReq.params;
    const tenantId = tenantReq.tenantId;
