...
const JobsPage = () => {
  ...
  const { jobs: jobsData, jobsStatistics, isLoading, getJobsError } = useAppSelector((state) => state.company);
  ...
  const activeJobs = jobsStatistics?.activeJobs ?? allJobs.filter((j) => j.status === "Active").length;
  const inactiveJobs = jobsStatistics?.inactiveJobs ?? allJobs.filter((j) => j.status === "Inactive").length;
        ...
        return (
          <span
            className={`px-2 py-1 h-6 flex items-center justify-center rounded-full border ${
              isActive
                ? "bg-[#fff] border-[#10B981] text-[#10B981]"
                : "bg-[#fff] border-[#EF4444] text-[#EF4444]"
            }`}
          >
    ...
    <div className="w-full px-4 sm:px-0">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <button
          ...
          className="h-[44px] cursor-pointer flex items-center justify-center gap-2 px-4 py-3 bg-[#0E1114] text-white rounded-lg hover:bg-black/80 transition-colors font-poppins font-semibold text-sm sm:text-button whitespace-nowrap w-full sm:w-auto"
        >
      ...
      <div className="flex flex-col bg-white border border-[#DEE0E3] rounded-lg overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 p-4 sm:p-5 border-b border-[#DEE0E3]">
          <div className="flex-1 max-w-[373px] relative">
            <div className="flex items-center w-full px-3 py-2 sm:py-4 h-[44px] sm:h-[52px] rounded-lg border border-[#DEE0E3] bg-white">
              <input
                type="text"
              ...
              />
              <Search className="w-5 h-5 sm:w-6 sm:h-6 text-primary flex-shrink-0 ml-2" />
            </div>
          </div>
          <div className="relative">
            <button
              ...
              className="h-[44px] sm:h-[50px] cursor-pointer flex items-center justify-center gap-2 px-4 py-2 sm:py-3 border border-[#8A949E] text-[#2E3338] rounded-lg hover:bg-gray-50 transition-colors font-poppins font-semibold text-sm sm:text-button whitespace-nowrap"
            >
          ...
          <>
            {isLoading && jobsData && (
              <div className="flex items-center justify-center p-2 bg-blue-50 border-b border-blue-200">
                <p className="text-xs text-blue-600 font-sf-pro">Refreshing jobs...</p>
              </div>
            )}
      ...
      <EditJobModal
        ...
        jobData={editingJob ? {
          ...
          emailConfig: {
            invoices: {
              includeSubmitter: editingJob.includeInvoiceSubmitter ?? true,
              recipients: editingJob.invoiceRecipients || [],
              cc: editingJob.invoiceCcRecipients || [],
            },
            dailyLogs: {
              includeSubmitter: editingJob.includeDailyLogSubmitter ?? true,
