...
export default function CompanyLayout({
  ...
  const [configureCOIAlerts, setConfigureCOIAlerts] = useState<string | null>(
    null
  );
      ...
      <main className="w-full max-w-[1362px] mx-auto px-4 sm:px-6 md:px-8 lg:px-[39px]">
        <div className="w-full min-h-[calc(100vh-100px)] bg-[#F3F5F7]">
          <div className="w-full mx-auto lg:pr-0 pt-6 sm:pt-8 md:pt-[34px] pb-8">
            ...
            {/* Header Container */}
            {configureCOIAlerts === 'true' ? null : (
              <div className="flex flex-col px-4 sm:px-0 gap-4 sm:gap-6 mb-6 sm:mb-8">
                ...
                <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto pb-2 -mx-4 sm:mx-0 px-4 sm:px-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                  {tabs.map((tab) => (
                    <Link
                      key={tab.id}
                      href={tab.path}
                      className={`px-3 cursor-pointer sm:px-4 py-2 sm:py-3 rounded-lg font-sf-pro font-medium text-sm sm:text-label transition-colors whitespace-nowrap shrink-0 ${
                        activeTab === tab.id
