...
import { updateJobFailure } from "@/Redux/reducers/company";
...
const EditJobModal: React.FC<EditJobModalProps> = ({
  ...
  const { isUpdatingJob, updateJobSuccess, updateJobError } = useAppSelector((state) => state.company);
  ...
  // Populate form when jobData changes
  useEffect(() => {
    if (jobData) {
      ...
      if (jobData.emailConfig) {
        setIncludeInvoiceSubmitter(
          jobData.emailConfig.invoices?.includeSubmitter ?? true
        );
        setInvoiceRecipients(jobData.emailConfig.invoices?.recipients || []);
        setInvoiceCC(jobData.emailConfig.invoices?.cc || []);

        setIncludeDailyLogSubmitter(
          jobData.emailConfig.dailyLogs?.includeSubmitter ?? true
        );
        setDailyLogRecipients(jobData.emailConfig.dailyLogs?.recipients || []);
        setDailyLogCC(jobData.emailConfig.dailyLogs?.cc || []);
      }
    }
  }, [jobData]);
  ...
  useEffect(() => {
    if (updateJobSuccess) {
      ...
      dispatch(updateJobFailure(false))
      // Small delay to allow success message to be seen
      setTimeout(() => {
        onClose();
      }, 100);
    }
  }, [updateJobSuccess, onClose, onSubmit]);

  // Reset error when modal closes
  useEffect(() => {
    if (!isOpen && updateJobError) {
      dispatch(updateJobFailure(""));
    }
  }, [isOpen, updateJobError, dispatch]);

  const handleSubmit = () => {
    // Validate required fields
    if (!jobName.trim() || !jobData?.id) {
      return;
    }
    ...
    if (activeTab === "email-config") {
      ...
      dispatch(companyActions.updateJobRequest(jobData.id, payload));
    }
  };
      ...
      <div className="flex flex-col gap-8 px-8 pt-6 pb-8">
        ...
        {updateJobError && (
          <div className="p-3 bg-[#FFEDED] border border-[#EF4444] rounded-lg">
            <p className="text-sm text-[#EF4444] font-sf-pro">{updateJobError}</p>
          </div>
        )}
        ...
        <div className="flex items-stretch gap-8">
          ...
          <button
            ...
            disabled={isUpdatingJob || !jobName.trim()}
