...
export const PricingCard = ({
  ...
  >
    ...
    <Link href={ctaLink || "/"}>
    <button
      className={`w-full cursor-pointer py-3 rounded-lg font-semibold text-sm transition-all duration-200 mb-8 ${
        isHighlighted
          ? "bg-[#1F6F66] text-white hover:bg-[#185A52] shadow-lg shadow-teal-200"
          : buttonBorderGreen
            ? "bg-white hover:bg-[#1F6F66] hover:text-white border border-[#1F6F66] text-[#1B232A] "
            : "bg-white border border-gray-200 text-[#1B232A] hover:text-white hover:bg-[#1F6F66]"
      }`}
    >
      {ctaText}
    </button>
    ...
    <div className="space-y-4 flex-1 mb-4">
      {features.map((feature, i) => {
        ...
        const iconColor = isExcluded ? "#DC3545" : "#1F6F66";
